...
It's not true because less than a half of values lies to the right of 35
### 3.2
...
It's not true because a minority of values lies to the left of 10
### 3.3
...
About a half of values lies to the right of 30, about a half lies to the left
...
Explain your answer below:
```
Just one one success out of 1 000 000 observations seems to be not enough amount to reject the null hypothesis.
```
...
```{r}
age <- c(44, 55, 42, 64, 66, 42, 72, 56, 72, 54, 46, 48, 48, 52, 50, 66, 84)
```
...
```
H0: Verbs in the rhyming position are used in the same way as verbs in general, which means that probability for verbs to be used in the rhyming position is 17%

H1: Verbs in the rhyming position are used differently which means that probability for verbs to be used in the rhyming position is not equal 17%
```
...
```{r}
trials <- nrow(df[df$Decade == '1920s',])
sucsesses <- nrow(df[df$Decade == '1920s' & df$UPoS == 'VERB',])

binom.test(sucsesses, trials, p = 0.17)
```
...
```
p-value = 0.2297, p-value > 0,05 => We cannot reject the null hypothesis.
According to test results we cannot decide that in verses written in 1920s, verbs are used in the rhyming posision more often or less often than expected.
```
...
```{r}
trials18 <- nrow(df[df$Decade == '1820s',])
sucsesses18 <- nrow(df[df$Decade == '1820s' & df$UPoS == 'VERB',])

binom.test(sucsesses18, trials18, p = 0.17)
```

Write down your general conlusions about data provided for both 1920s and 1820s data.
```
Despite the fact that in 1920s experiment probability for verbs to be used in the rhyming posision is higher than in 1820s experiment, in both cases p-value is more than 0,05. So for both experiements we cannot reject the null hypothesis.
```
...
```
H1: The population mean of vowel duration in speaker `shg05` does not equal 73 ms (it's less or more than 73).
```

### 8.2
Perform a one-sample t-test. 
```{r}
df2 <- read.csv("https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv")
shg05 <- df2[df2$speaker == 'shg05',]
...
Interpret results. 
```
p-value = 0.05172, p-value > 0,05, so we cannot reject the null hypothesis.
```
